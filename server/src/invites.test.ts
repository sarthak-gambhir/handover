...
function newMember(session: ReturnType<typeof newOwnerSession>['session'], name = 'Other') {
  return store.admitKnocker(session, store.addKnocker(session, name).knocker.knock_id)!;
}
...
describe('POST /api/sessions/:slug/invites/:code (redeem)', () => {
  it('redeems an invite, joins as a member, and sets a member cookie', async () => {
    ...
    expect(setCookie?.some((c) => c.startsWith(`${cookieName(slug)}=`))).toBe(true);
  });
