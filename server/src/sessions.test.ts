...
describe("Store — byte reservation", () => {
  ...
  it("memory-cap race: only the fitting subset of concurrent reserves succeed", () => {
    ...
    const results = [0, 1, 2, 3, 4].map(() =>
      store.reserveBytes(session, chunk)
    );
...
describe("Store — sweeper", () => {
  ...
  it("moves hung transfers to expired per-state and emits transfer:expired", () => {
    const store = new Store();
    const { session, ownerUserId } = store.createSession();
    const m = store.admitKnocker(
      session,
      store.addKnocker(session, "B").knocker.knock_id
    )!;
    const t = store.createTransfer(session, ownerUserId, m.user_id, [
      { name: "a", size: 1 },
    ]);
    const now = Date.now();
    session.last_activity = now;
...
describe("Store — transfer cancellation on departure", () => {
  it("cancels every non-terminal transfer a user is part of", () => {
    ...
    const t1 = store.createTransfer(session, ownerUserId, m.user_id, [
      { name: "a", size: 1 },
    ]);
    const t2 = store.createTransfer(session, m.user_id, ownerUserId, [
      { name: "b", size: 1 },
    ]);
    ...
    expect(cancelled.map((c) => c.transfer.transfer_id)).toEqual([
      t1.transfer_id,
    ]);
