...
describe("requireMember", () => {
  ...
  it("403 when a cookie for slug A is presented against slug B", () => {
    ...
    let called = false;
    requireMember(
      makeReq(b.session.slug, cookies),
  ...
  it("passes for a valid member and refreshes the cookie (sliding window)", () => {
    ...
    const req = makeReq(session.slug, {
      [cookieName(session.slug)]: ownerToken,
    });
    const res = makeRes();
    let called = false;
    ...
    const refreshed = res.cookies.find(
      (c) => c.name === cookieName(session.slug)
    );
...
describe("requireOwner", () => {
  ...
  it("403 for a member who is not the owner", () => {
    ...
    let called = false;
    requireOwner(
      makeReq(session.slug, {
        [cookieName(session.slug)]: member.session_token,
      }),
      res as unknown as Response,
      () => {
        called = true;
